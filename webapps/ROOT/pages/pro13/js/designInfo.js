	...
	function fixFilename(fp){
		...
		function initDataCallBack(obj){
			  ...
			  $("#view_measure_cust_name").html(o.cust_name);
			  $("#view_measure_cust_mobile").html(o.cust_mobile);
			  $("#view_measure_cust_sex" ).html(top.getCodeName("SEX",o.cust_sex));
			  $("#view__measure_planinvest_money").html(o.measure_planinvest_money);
			  $("#lab_measure_cust_name").html(o.cust_name);
			  $("#measure_cust_name").html(o.cust_name);
			  $("#lab_measure_cust_sex").html(top.getCodeName("SEX",o.cust_sex));
			  $("#measure_cust_sex").html(top.getCodeName("SEX",o.cust_sex));
			  $("#measure_cust_mobile").html(o.cust_mobile);
			  ...
			  if ( o.requireInfo ) {
				...
				$("#view_cust_receiver").html(o.requireInfo.cust_receiver);
				...
				if(o.requireInfo.planinvest_money){
				  $("#view__measure_planinvest_money").html(o.requireInfo.planinvest_money);
				}else{
				  $("#view__measure_planinvest_money").html("0");
				}
				if(o.requireInfo.major_material){
				  $("#view_measure_major_material").html(o.requireInfo.major_material);
				}else{
				  $("#view_measure_major_material").html("0");
				}
				...
				if(o.requireInfo.room_count){
				  $("#lab_measure_room_count").html( o.requireInfo.room_count);
				}else{
				  $("#lab_measure_room_count").html("0");
				}
				if(o.requireInfo.hall_count){
				  $("#lab_measure_hall_count").html( o.requireInfo.hall_count);
				}else{
				  $("#lab_measure_hall_count").html("0");
				}
				if(o.requireInfo.bath_count){
				  $("#lab_measure_bath_count").html( o.requireInfo.bath_count);
				}else{
				  $("#lab_measure_bath_count").html("0");
				} 
			...
			if(o.measurehouse){
				...
				if(o.measurehouse.partition_position){
				  $("#view_partiition_position_text").show();
				}else{
				  $("#view_partiition_position_text").hide();
				}
				...
				$("#view_many_members").html(o.measurehouse.many_members);
	...
	function checkSaveMeasureHouse(){
		var msg="";
	    if ( $("#many_members").val() != '' && !IsFloat( $("#many_members").val() )) msg += "[家庭成员人数]应为数字类型<br>";
	...
	function optMeasureHouseCallback(obj){
       try{
        if ( obj.status == true ){
			...
			$("#view_many_members").html($("#many_members").val()  );
	...
	function initmeasureRoomInfo(roomInfoList){
		...
		if(roomInfoList!=null&&roomInfoList.length>0){
			for(var i=0;i<roomInfoList.length;i++){
				...
				ht+="<img src='../../img/edit.gif'  style='cursor:pointer;' title='编辑' onclick='editRoomInfo(\""+roomInfoIndex+"\")'>&nbsp;<img src='../../img/delete.gif'  style='cursor:pointer;' title='取消' onclick='deleteRoomInfo(\""+roomInfoIndex+"\")'>";
	...
	function createDesignPlanStatusTable(obj){
		try{
		 ...
		 for (var i=0;i<designPlanList.length;i++) {
			dis_days=(designPlanList[i].plan_end_date-designPlanList[i].plan_begin_date)/(24*60*60000);
			fact_days=(designPlanList[i].fact_end_date-designPlanList[i].fact_begin_date)/(24*60*60000);
	...
	function saveDesignAgreement(){
	   ...
	   if(!IsFloat($("#price").val())){top.showInfoWinWarn("设计单价类型错误，请重新输入！");return;}
	   ...
	   switch($("input[type=radio]:checked").val()){
              ...
              case "1":
                ...
                if($("#ext17").val()!="" && !IsFloat($("#ext17").val())){top.showInfoWinWarn("不含公摊外墙面积类型错误，请重新输入！");return;}
                break;
              case "2":
...
//附件上传成功后附件列表刷新
function saveDesignAgrFileCallBack(obj){
	  var aoData=[];
...
function getDesignSchemeCallback(obj){
	...
	//审核设计方案
	function examineDesignScheme(id,status,remindId){
	 design_id=id;
	...
	function doExamineDesignScheme(){
      ...
      buttons: {
        "确定": function() {
		  ...
		  var aoData=[];
	      aoData.push( { "name": "designScheme.design_id", "value": design_id } );
	      aoData.push( { "name": "designScheme.examine_status", "value": design_status } );
		  aoData.push( { "name": "designScheme.design_bz", "value":bz} );
		  //审核结果通知
		  aoData.push( { "name": "custInfo.code", "value":cust_code} );//客户id   wwww
		  aoData.push( { "name": "custRequire.require_manager", "value":$("#measure_designer").val()} );//设计师id
		  if(remind_id){
