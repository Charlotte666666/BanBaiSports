function productInitData(){
	...
	function addProductCustomize(){ 
		...
		$("#productcustomize tr:eq("+Customizetrnum+")").after("<tr class='tr_data' height='35px' id='customize"+Customizetrid+"'><td><input type='text' size='15' id='product_name"+Customizetrid+"'/></td><td><input type='text' size='15' id='money"+Customizetrid+"'/></td><td><input type='text' size='15' id='customize_date"+Customizetrid+"' onclick='new Calendar().show(this);'/></td><td><input type='text' size='15' id='receive_date"+Customizetrid+"' onclick='new Calendar().show(this);'/></td><td><input type='text' size='15' id='install_date"+Customizetrid+"' onclick='new Calendar().show(this);'/></td><td><input type='text' size='15' id='customize_memo"+Customizetrid+"'/></td><td><a href=\"javascript:void(0)\" onclick='saveProductCustomize("+Customizetrid+")'>保存</a>||<a href=\"javascript:void(0)\" onclick='deleteProductCustomize("+Customizetrid+")'>删除</a></td></tr>");
		Customizetrnum++;
		Customizetrid++;
	}
	//删除产品定制行
			...
			for ( var i=0;i<objs.length;i++ ) {
				...
				if ( isExistProduct(ch_product) ) {
				   ...
				   // orderdetailList[orderdetailList.length]=;
					...
					ht+='</td>';
					ht+='<td>';
					ht+='<input type="text" size="5" id="product_brand_'+ch_product.product_id+'" value="'+top.getCodeName( "PRODUCT_BRAND", ch_product.product_brand)+'" disabled />';
					ht+='</td>';				 
					ht+='<td>';
		 ...
		 if(orderdetail.length>0){
			...
			for (var i=0;i<orderdetail.length;i++) {
				...
				if(orderdetail[i].parent_type==parentType){
			 	...
			 	$("#tdtype_"+orderdetail[i].type_id+"_product_brand").html(top.getCodeName("PRODUCT_BRAND",productList[i].product_brand));
		...
		//判断之前值是否为空
			...
			for(var i=0;i<productList.length;i++){
				if(proId==productList[i].product_id){
					...
					$("#tdtype_"+curProductIndex+"_product_brand").html(top.getCodeName("PRODUCT_BRAND",productList[i].product_brand));
   ...
   function getOneProductRecord(proId,curProductIndex){  
		for(var i=0;i<productList.length;i++){
			if(proId==productList[i].product_id){
				...
				$("#type_"+curProductIndex+"_sale_price").html(productList[i].price);
				$("#type_"+curProductIndex+"_discount").html(productList[i].discount);
				$("#type_"+curProductIndex+"_product_brand").html(top.getCodeName("PRODUCT_BRAND",productList[i].product_brand));
...
//---------------------------产品类型树生成-----------------------
	...
	function getOldProductList(){
		...
		if(orderdetail.length>0){
			for (var i=0;i<orderdetail.length;i++) {
				...
				ht+="<td>"+top.getCodeName("PRODUCT_BRAND",orderdetail[i].product_brand)+"</td>";
	...
	function displayCertainProduct(id){
		...
		for(var i=0;i<productList.length;i++){
			if(proId==productList[i].product_id){
				...
				$("#tdnew_product_brand").html(top.getCodeName("PRODUCT_BRAND",productList[i].product_brand));
	...
	//新增
		...
		if($("#change_type"+i).val()!='01'){
			...
			ht+="<td><input type='hidden' value='"+orderdetail[i].product_brand+"' id='old_product_brand'/>"+top.getCodeName("PRODUCT_BRAND",orderdetail[i].product_brand)+"</td>";
