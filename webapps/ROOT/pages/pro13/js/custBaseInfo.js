   ...
   function baseInitData(){
       ...
       try{
        if ( obj.status == true ){
			...
			if ( cust_base_info_id == 0||require_id=='' ) {
				require_id=obj.msg;
				cust_base_info_id=$("#cust_code").val() ;
				...
				//添加首次首发内容
				var ht='<li><a onclick="changBaseBtclick(\''+callbackNum+'\',\'callback\')" href="#cust_base_tabs-'+callbackNum+'"><span>'+callBackNumber[0]+'次沟通</span></a></li>';
				$('#custManageWorkTab').append(ht);//添加第几次回访标题标签
    ...
    function refreshCustBase(){
		...
		$("#top_cust_code").html($("#cust_code").val());
		...
		if($( "#other_from_type" ).val()!=null&&$( "#other_from_type" ).val()!=''){
			...
			$("#view_cust_manager").html(top.getUserNameByCode($( "#cust_manager").val()));
			  ...
			  $("#view_interpose_type").html(top.getCodeName("INTERPOSE_TYPE", $("#interpose_type").val()));
				...
				/*if(top.g_isPermit('09_alter_designer')&&!top.g_isPermit('cust_edit')){
					$("#view_designer_code").html( top.getUserNameByCode($("#view_designer_code_val").val()) );
					$("#view_sapn_designer_code").hide();
					$("#view_designer_code").show();
					$("#designer_code").val($( "#view_designer_code_val").val());
				}*/
			}
    ...
    function changBaseBtclick(tab_id,bOrA){
	cur_tab_id=tab_id; 
	/*if(tab_id==1&&bOrA=='callInfo'){
			$("#cbEditBt").attr("onclick","showSalesStrategy()");
			$("#cbSaveBt").attr("onclick","AddSalesStrategy()");
			$("#cbReturnBt").attr("onclick","doCancelSalesStrategy()");
			editCBModelBt();
	}else{*/
			if(bOrA=='analysisInfo'){
				$("#cbSaveBt").attr("onclick","addCustAnalyze()");
				$("#cbReturnBt").attr("onclick","doCancelCustAnalyze()");
				$("#cbEditBt").attr("onclick","showCustAnalyze()");
				//editCBModelBt();
				
				if(tab_id<callbackNum-1){//编辑
					$("#view_custAnalysis"+tab_id).show();
					$("#custAnalysis"+tab_id).hide();
					$("#callBackInfo"+tab_id).hide();
					editCBModelBt();
				}else if(callbackNum!=2){//保存
					$("#view_custAnalysis"+tab_id).hide();
					$("#custAnalysis"+tab_id).show();
					$("#callBackInfo"+tab_id).hide();
					saveCBModelBt();
				}else{//编辑
					$("#view_custAnalysis"+tab_id).hide();
					...
					$("#cbSaveBt").attr("onclick","saveMoreCallBack()");
					$("#cbReturnBt").attr("onclick","doCancelCallBack()");
					$("#cbEditBt").attr("onclick","showCallBack()");
					editCBModelBt();
				if(tab_id<callbackNum-1){//编辑
	...
	//店长、店面经理
		...
		if(top.g_isPermit('cust_edit')){
			if(top.g_isPermit('09_alter_cust_manager')){
				$("#tdcust_manager").show();
				$("#edit_tdcust_manager").hide();
			}/*if(top.g_isPermit('09_alter_designer')){
				$("#span_designer_code").show();
				$("#edit_span_designer_code").hide();
			}*/
			...
			if(top.g_isPermit('09_alter_cust_manager')){
				...
				$("#view_sapn_designer_code").html(top.getAllDesignerSelect("view_designer_code_val",$("#designer_code").val()));
				$("#view_sapn_designer_code").show();
				$("#view_designer_code").hide();
			}*/
		}
    }
  ...
  //客户基本信息保存、编辑、取消按钮隐藏显示
	...
	function baseInfoCallback(obj){
	   try{
			...
			if(o.cbmList){
				...
				if(o.cbmList.length==0){
					//if(o.salesstrategy.require_id!=0){
						//AddMoreCallBack();
						//addCallBackHtml();//生成每次回访的页面html框架
						ht='<li><a onclick="changBaseBtclick(\''+callbackNum+'\',\'callback\')" href="#cust_base_tabs-'+callbackNum+'"><span>'+callBackNumber[i]+'次回访</span></a></li>';
						$('#custManageWorkTab').append(ht);//添加第几次回访标题标签
						//AddMoreCallBack();
						addCallBackHtml();//生成每次回访的页面html框架
						AddMoreCallBack();
					...
					for(var j=0;j<=o.cbmList.length;j++){
						if(j>8){
							top.showInfoWinError("回访次数最大限度为十次！");
							//top.showInfoWinError("处理异常:"+e.message);
							break;
						}
						if(j!=o.cbmList.length){i=j;
							name = fixFilename(o.cbmList[i].file_path);
						}
						addCallBackHtml();//生成每次回访的页面html框架
						ht='<li><a onclick="changBaseBtclick(\''+callbackNum+'\',\'callback\')" href="#cust_base_tabs-'+callbackNum+'"><span>'+callBackNumber[j]+'次回访</span></a></li>';
						$('#custManageWorkTab').append(ht);//添加第几次回访标题标签
						ht='';
						...
						ht+='<td width="25%" align="center">本次来店时间';
						if(j!=o.cbmList.length){
							ht+='<input type="hidden" id="next_id'+callbackNum+'" value="'+o.cbmList[i].next_id+'"/>';
						}else{
							ht+='<input type="hidden" id="next_id'+callbackNum+'"/></td>';
						}
						ht+='<td width="25%" align="left"><input type="text" id="come_time'+callbackNum+'" onclick="new Calendar().show(this);" value="'+top.getTimeStr(o.cbmList[i].come_time,true)+'"/></td>';
						ht+='<td width="25%" align="center">预约下次来店时间</td>';
						ht+='<td width="25%" align="left"><input type="text" id="next_time'+callbackNum+'" onclick="new Calendar().show(this);" value="'+top.getTimeStr(o.cbmList[i].next_time,true)+'"/></td>';
							
						ht+='</tr>';
						ht+='<tr class="tr_data" height="35px">';
						ht+='<td align="center">客户满意度<span style="color:red;">(*)</span></td>';
						...
						ht+='<tr class="tr_data" height="35px">';
							...
							ht+='<td align="left"><input type="text" id="callback_time'+callbackNum+'" onclick="new Calendar().show(this);" value="'+top.getTimeStr(o.cbmList[i].callback_time,true)+'"/></td>';
							ht+='<td align="center" >结果达成</td>';
							ht+='<td align="left" id="tdresult_status'+callbackNum+'"></td>';
						ht+='</tr>';
						ht+='<tr class="tr_data" height="35px">';
							ht+='<td align="center">预约量房时间</td>';
							ht+='<td align="left" ><input type="text" id="measure_date'+callbackNum+'" onclick="new Calendar().show(this);" value="'+top.getTimeStr(o.cbmList[i].measure_date,true)+'"/></td>';
							measureHouseDate=o.cbmList[i].measure_date;
							ht+='<td align="center">客户意向</td>';
							ht+='<td align="left" id="tdcust_intention'+callbackNum+'"></td>';
						ht+='</tr>';
						ht+='<tr class="tr_data" height="35px">';
							ht+='<td align="center" >客户意见或建议</td>';
							ht+='<td align="left" colspan="3"><textarea style="width:680px" id="cust_suggest'+callbackNum+'" rows="3" cols="70">'+o.cbmList[i].cust_suggest+'</textarea></td>';
						ht+='</tr>';
						ht+='<tr class="tr_data" height="35px">';
							ht+='<td align="center" >销售策略</td>';
							ht+='<td align="left" colspan="3"><textarea style="width:680px" id="sales_measure'+callbackNum+'" rows="3" cols="70">'+o.cbmList[i].sales_measure+'</textarea></td>';
						ht+='</tr>';
						ht+='<tr class="tr_data" height="35px">';
						...
						ht+='<tr class="tr_data" height="45px">';
							...
							ht+='<td align="left" colspan="2"><textarea style="width:99%"id="msg_content'+callbackNum+'" cols="80" rows="3">'+o.cbmList[i].msg_content+'</textarea></td>';
						...
						ht+='<td align="left"><textarea id="cusm_callback_info'+callbackNum+'" rows="2" style="width:100%">'+o.cbmList[i].cusm_callback_info+'</textarea></td>';
						ht+='</tr>';
						ht+='<tr class="tr_data">';
						ht+='<td class="table_property_title" style="width:110px;">设计师</td>';
						ht+='<td align="left"><textarea id="des_callback_info'+callbackNum+'" rows="2" style="width:100%">'+o.cbmList[i].des_callback_info+'</textarea></td>';
						ht+='</tr>';
						ht+='</table>';
						ht+='</td>';
						ht+='</tr>';
						...
						ht+='<tr class="tr_data" height="35px">';
						ht+='<td align="left"><textarea id="custom_tactics'+callbackNum+'" rows="5" style="width:800px">'+o.cbmList[i].custom_tactics+'</textarea></td>';
						ht+='</tr>';
						ht+='</table>';
						...
						ht+='<tr class="tr_data" height="35px">';
						ht+='<td width="25%" align="center">本次来店时间';
						ht+='<td width="25%" align="left" id="view_come_time'+callbackNum+'">'+top.getTimeStr(o.cbmList[i].come_time,true)+'</td>';
						ht+='<td width="25%" align="center" >预约下次来店时间</td>';
						ht+='<td width="25%" align="left" id="view_next_time'+callbackNum+'" >'+top.getTimeStr(o.cbmList[i].next_time,true)+'</td>';
							
						ht+='</tr>';
						ht+='<tr class="tr_data" height="35px">';
						ht+='<td align="center">客户满意度</td>';
						if(o.cbmList[i].unsati_reason!=''){
							ht+='<td align="left" colspan="3"><span id="view_cust_sati'+callbackNum+'" style="padding:0px 15px 0px 0px">'+top.getCodeName("CUST_UNSATI_REASON",o.cbmList[i].unsati_reason)+'</span></td>';
						}else{
							ht+='<td align="left" colspan="3"><span id="view_cust_sati'+callbackNum+'" style="padding:0px 15px 0px 0px">'+top.getCodeName("APPRAISE_DESIGNER",o.cbmList[i].cust_sati)+'</span></td>';
						}ht+='</tr>';
						
						ht+='<tr class="tr_data" height="35px">';
							ht+='<td align="center" >电话回访时间</td>';
							ht+='<td align="left" id="view_callback_time'+callbackNum+'"> '+top.getTimeStr(o.cbmList[i].callback_time,true)+'</td>';
							ht+='<td align="center" >结果达成</td>';
							ht+='<td align="left" id="view_result_status'+callbackNum+'">'+top.getCodeName("RESULT_STATUS",o.cbmList[i].result_status)+'</td>';
						ht+='</tr>';
						ht+='<tr class="tr_data" height="35px">';
							ht+='<td align="center">预约量房时间</td>';
							ht+='<td align="left" id="view_measure_date'+callbackNum+'" >'+top.getTimeStr(o.cbmList[i].measure_date,true)+'</td>';
							ht+='<td align="center">客户意向</td>';
							ht+='<td align="left" id="view_cust_intention'+callbackNum+'">'+top.getCodeName("INTENTION_EVALUATE",o.cbmList[i].cust_intention)+'</td>';
						ht+='</tr>';
						ht+='<tr class="tr_data" height="35px">';
							ht+='<td align="center" >客户意见或建议</td>';
							ht+='<td align="left" colspan="3" id="view_cust_suggest'+callbackNum+'" >'+o.cbmList[i].cust_suggest+'</td>';
						ht+='</tr>';
						ht+='<tr class="tr_data" height="35px">';
							ht+='<td align="center" >销售策略</td>';
							ht+='<td align="left" colspan="3" id="view_sales_measure'+callbackNum+'" >'+o.cbmList[i].sales_measure+'</td>';
						ht+='</tr>';
						ht+='<tr class="tr_data" height="35px">';
							ht+='<td align="center">其他销售策略附件</td>';
							ht+='<td align="left" colspan="3"><div id="view_file_path'+callbackNum+'"></div></td>';
						ht+='</tr>';
						ht+='<tr class="tr_data" height="45px">';
							ht+='<td align="center">短信内容</td>';
							ht+='<td align="left" colspan="3" id="view_msg_content'+callbackNum+'" >'+o.cbmList[i].msg_content+'</td>';
						ht+='</tr>';
							ht+='<tr class="tr_data" height="35px">';
						...
						ht+='<td align="left" id="view_cusm_callback_info'+callbackNum+'" >'+o.cbmList[i].cusm_callback_info+'</td>';
						ht+='</tr>';
						ht+='<tr class="tr_data">';
						ht+='<td class="table_property_title" style="width:165px;">设计师</td>';
						ht+='<td align="left" id="view_des_callback_info'+callbackNum+'">'+o.cbmList[i].des_callback_info+'</td>';
						ht+='</tr>';
						ht+='</table>';
						ht+='</td>';
						ht+='</tr>';
						...
						ht+='<table border="1" cellpadding="3" cellspacing="1" width="100%" align="center" style="background-color: #b9d8f3;">';
						ht+='<tr class="tr_data" height="35px">';
						ht+='<td align="left" id="view_custom_tactics'+callbackNum+'">'+o.cbmList[i].custom_tactics+'</td>';
						ht+='</tr>';
						ht+='</table>';
						//ht+='</table>';
						...
						if(i!=(callbackNum-3)){
							...
							$("#view_file_path"+callbackNum).html('&nbsp;&nbsp;<a href="'+top.getUrlBase()+'/download?filePath='+o.cbmList[i].file_path+'+"&fileName="'+name+'">'+name+'</a>');
						}
						...
						$("#tdcust_sati"+callbackNum).html( top.getCodeSelectHtml("APPRAISE_DESIGNER","cust_sati"+callbackNum,true,o.cbmList[i].cust_sati,"cust_satiChange("+callbackNum+")"));
						$("#tdunsati_reason"+callbackNum).html( top.getCodeSelectHtml("CUST_UNSATI_REASON","unsati_reason"+callbackNum,true,o.cbmList[i].cust_sati) );
		...
		// 权限控制
		/*if ( !top.g_isPermit('cust_edit') ) {
			$("#cust_base_tabs-0 textarea").attr('disabled',true);
			$("#cust_base_tabs-0 input").attr('disabled',true);
			$("#cust_base_tabs-0 input[type=checkbox]").attr("disabled", true) ;
			$("#cust_base_tabs-0 input[type=radio]").attr("disabled", true) ;
			$("#cust_base_tabs-0 select").attr("disabled", true);
		}
		if ( !top.g_isPermit('save_cust_analyze') ) {
			$("#cust_base_tabs-1 textarea").attr('disabled',true);
			$("#cust_base_tabs-1 input[type=checkbox]").attr("disabled", true) ;
			$("#cust_base_tabs-1 select").attr("disabled", true) ;
			$('#savecustanalyzeBtn').hide();
		}
		if ( !top.g_isPermit('save_sales_strategy') ) {
			$("#cust_base_tabs-2 textarea").attr('disabled',true);
			$("#cust_base_tabs-2 select").attr("disabled", true) ;
			$('#savesalesstrategyBtn').hide();
		}
		if (!top.g_isPermit('save_design_plan') ) {
			$("#cust_base_tabs-3 textarea").attr('disabled',true);
			$("#cust_base_tabs-3 input").attr('disabled',true);
			$("#cust_base_tabs-3 select").attr("disabled", true) ;
	...
	//左侧'客情分析'与'回访信息'菜单
		var ht='<div id="cust_base_tabs-'+callbackNum+'" width="95%">';
		//ht+='<table style="width:100%;height:100%;padding:5px;spacing:0;">';
		//ht+='<tr class="tr_data" style="width:100%;height:100%;text-align:left">';
		//ht+='<td style="width:15%;border:1px;border-style: dotted;border-color: #A0CFEC;" valign="top">';
		//ht+='<table border="1" cellpadding="3" cellspacing="1" width="100%" align="right" style="background-color: #b9d8f3;" id="table_sms_type">';
		//ht+='<tr class="tr_data" height="35px">';
		//ht+='<td style="cursor:pointer" id="left_menu1_'+callbackNum+'" class="versionStyle2" align="center" onclick="showCallBackInfo(\'analysisInfo\')">客情分析</td>';
		//ht+='</tr>';
		//ht+='<tr class="tr_data" height="35px">';
	    //ht+='<td style="cursor:pointer" id="left_menu2_'+callbackNum+'" class="versionStyle" align="center" onclick="showCallBackInfo(\'callInfo\')">回访信息</td>';
		//ht+='</tr>';
		//ht+='</table>';
		//ht+='</td>';
		//右侧具体内容
		//ht+='<td id="treeDetail'+callbackNum+'" style="width:85%;border:1px;border-style: dotted dotted dotted none;border-color: #A0CFEC;" valign="top">';
		//ht+='<div style="display:none" id="custAnalysis'+callbackNum+'"></div>';
		//ht+='<div style="display:block" id="view_custAnalysis'+callbackNum+'"></div>';
...
//--------------------------客情分析--------------------------
	...
	function createCustAnalysisHtml(){
		var ht='';
		ht+='<input type="hidden" id="analysis_id'+custAnalysisNum+'"></input>';
		ht+='<table border="0" cellpadding="3" cellspacing="1px" width="100%" align="center" style="background-color: #b9d8f3; border-collapse:collapse;" >';
		
		ht+='<tr class="tr_data" >';
		ht+='<td width="20%" align="center" style="border-left:1px solid  #b9d8f3;border-bottom:1px solid  #b9d8f3;border-top:1px solid  #b9d8f3;">客户需求与失效分析</td>';
		ht+='<td align="left" width="80%" style="padding:0px;border-collapse:collapse;">';
		ht+='<table cellpadding="3px" cellspacing="1px" width="100%" align="center" style="background-color: #b9d8f3;">';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left" colspan="3">';
		ht+='<table width="100%" frame=void>';
		ht+='<tr>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="is_consume_ability'+custAnalysisNum+'" value="有消费能力"/>有消费能力</br>';
		ht+='</td>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="is_consume_ability'+custAnalysisNum+'" value="有消费意愿"/>有消费意愿</br>';
		ht+='</td>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="is_consume_ability'+custAnalysisNum+'" value="客户是否缴纳设计费或订金"/>客户是否缴纳设计费或订金</br>';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>	';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px" style="padding:1px">';
		ht+='<td border="0" width="15%" align="left" style="padding:5px;border:0px" colspan="3">';
		ht+='<label style="font-weight:bold;">客户关注点：</label>';
		ht+='<table width="100%" frame=void>';
		ht+='<tr>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="cust_focus'+custAnalysisNum+'" value="对设计有特殊需求"/>对设计有特殊需求';
		ht+='</td>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="cust_focus'+custAnalysisNum+'" value="对施工质量有特殊需求"/>对施工质量有特殊需求';
		ht+='</td>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="cust_focus" value="对设计师有特殊要求"/>对设计师有特殊要求';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr>';
		ht+='<td width="33%">	';
		ht+='<input type="checkbox" name="cust_focus'+custAnalysisNum+'" value="对施工周期有较高要求"/>对施工周期有较高要求';
		ht+='</td>';
		ht+='<td colspan="2">';
		ht+='<input type="checkbox" name="cust_focus'+custAnalysisNum+'" value="对报价有特别在意"/>对报价有特别在意';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px" style="padding:1px">';
		ht+='<td border="0" width="15%" align="left"  colspan="3" style="padding:5px;border:0px">';
		ht+='<label style="font-weight:bold;">客户有可能丢失的原因是：</label>';
		ht+='<table width="100%" frame=void>';
		ht+='<tr>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="cust_lost_reason'+custAnalysisNum+'" value="不认可设计费"/>不认可设计费  ';
		ht+='</td>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="cust_lost_reason'+custAnalysisNum+'"  value="设计方案不满意"/>设计方案不满意  ';
		ht+='</td>';
		ht+='<td width="33%">	';
		ht+='<input type="checkbox" name="cust_lost_reason'+custAnalysisNum+'" value="设计师个人问题"/>设计师个人问题';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="cust_lost_reason'+custAnalysisNum+'" value="样板间不满意"/>样板间不满意 ';
		ht+='</td>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="cust_lost_reason'+custAnalysisNum+'" value="报价偏高"/>报价偏高 ';
	    ht+='</td>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="cust_lost_reason'+custAnalysisNum+'" value="服务不满意"/>服务不满意';
	    ht+='</td>';
		ht+='</tr>';
		ht+='<tr>';
		ht+='<td width="33%">';
		ht+='<input type="checkbox" name="cust_lost_reason'+custAnalysisNum+'" value="工程周期"/>工程周期 ';
		ht+='</td>';
		ht+='<td colspan="2">';
		ht+='<input type="checkbox" name="cust_lost_reason'+custAnalysisNum+'" value="其他竞争对手"/>其他竞争对手';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr>';
		ht+='<td width="33%" colspan="3">';
		ht+='<input type="checkbox" name="cust_lost_reason'+custAnalysisNum+'" onclick="doOtherCustLostReason()" value="其他"/>其他<span id="span_other_cust_lost_reason" style="display:none"><input type="text" id="other_cust_lost_reason"/></span>';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td border="0" width="15%" align="left"  colspan="3">';
		ht+='<label style="font-weight:bold;">客户选择我们公司的可能性在：</label>';
		ht+='<input type="checkbox" name="choose_reason'+custAnalysisNum+'" value="设计"/>设计 <input type="checkbox" name="choose_reason'+custAnalysisNum+'""  value="服务"/>服务<input type="checkbox" name="choose_reason'+custAnalysisNum+'""  value="施工样板质量"/>施工样板质量';
	    ht+='<input type="checkbox" name="choose_reason'+custAnalysisNum+'"  value="品牌口碑"/>品牌口碑  <input type="checkbox" name="choose_reason'+custAnalysisNum+'""  value="报价"/>报价 <input type="checkbox" name="choose_reason'+custAnalysisNum+'""  value="其它"/>其它';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left"  colspan="3">';
		ht+='<label style="font-weight:bold;">客户对我们公司整体印象是：</label><span id="span_general1'+custAnalysisNum+'">'+top.getCodeSelectHtml("IMPRESSION","general1"+custAnalysisNum,true)+'</span>';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left"  colspan="3">';
		ht+='<label style="font-weight:bold;">客户对设计师的整体印象是：</label><span id="span_general2'+custAnalysisNum+'">'+top.getCodeSelectHtml("APPRAISE_DESIGNER","general2"+custAnalysisNum,true)+'</span>';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" >';
		ht+='<td width="15%" align="center" width="3%" style="border-left:1px solid  #b9d8f3;border-bottom:1px solid  #b9d8f3;">设计思路和平面布置</td>';
		ht+='<td align="left" width="45%" style="padding:0px;border-collapse:collapse;border-right:1px solid #b9d8f3" >';
		ht+='<table border="0" width="100%" align="center" style="background-color: #b9d8f3;border-collapse:collapse;">';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left" style="border-left:1px solid #b9d8f3;border-bottom:1px solid #b9d8f3;">';
		ht+='<input type="checkbox" name="general3'+custAnalysisNum+'" value="是否确定整体装修风格"/>是否确定整体装修风格';
		ht+='</td>';
		ht+='<td width="15%" align="left" style="border-left:1px solid #b9d8f3;border-bottom:1px solid #b9d8f3;">';
	    ht+='<input type="checkbox" name="general3'+custAnalysisNum+'" value="空间结构划分是否与客户确定"/>空间结构划分是否与客户确定';
		ht+='</td>';
		ht+='<td width="15%" align="left" style="border-left:1px solid #b9d8f3;border-bottom:1px solid #b9d8f3;">';
		ht+='<input type="checkbox" name="general3'+custAnalysisNum+'" value="客户是否有意愿缴纳设计费"/>客户是否有意愿缴纳设计费';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left" style="border-left:1px solid #b9d8f3;border-bottom:1px solid #b9d8f3;">';
		ht+=' <input type="checkbox" name="general3'+custAnalysisNum+'" value="通道设置保证有效使用面积"/>通道设置保证有效使用面积';
		ht+='</td>';
		ht+='<td width="15%" align="left" style="border-left:1px solid #b9d8f3;border-bottom:1px solid #b9d8f3;">';
		ht+='<input type="checkbox" name="general3'+custAnalysisNum+'" value="通风问题解决是否合理"/>通风问题解决是否合理';
		ht+='</td>';
		ht+='<td width="15%" align="left" style="border-left:1px solid #b9d8f3;border-bottom:1px solid #b9d8f3;">';
		ht+='<input type="checkbox" name="general3'+custAnalysisNum+'" value="采光问题解决是否合理"/>采光问题解决是否合理';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left" style="border-left:1px solid #b9d8f3;">';
		ht+='<input type="checkbox" name="general3'+custAnalysisNum+'" value="家具选择与风格是否协调"/>家具选择与风格是否协调';
		ht+='</td>';
		ht+='<td width="15%" align="left" style="border-left:1px solid #b9d8f3;">';
		ht+='<input type="checkbox" name="general3'+custAnalysisNum+'" value="装修结构与风格是否协调"/>装修结构与风格是否协调';
		ht+='</td>';
		ht+='<td width="15%" align="left" style="border-left:1px solid #b9d8f3;">';
		ht+=' <input type="checkbox" name="general3'+custAnalysisNum+'" value="色彩搭配与风格是否协调"/>色彩搭配与风格是否协调';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" >';
		ht+='<td width="15%" align="center" width="3%" style="border-left:1px solid  #b9d8f3;border-bottom:1px solid  #b9d8f3;">材料使用和性价比</td>';
		ht+='<td border="0" align="left" width="45%" style="padding:0px;border-collapse:collapse;">';
		ht+='<table border="1" cellpadding="3" cellspacing="1" width="100%" align="center" style="background-color: #b9d8f3;">';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left" >';
		ht+='<input type="checkbox" name="general4'+custAnalysisNum+'" value="装修项目设置是否合理"/>装修项目设置是否合理';
		ht+='</td>';
		ht+='<td width="15%" align="left" >';
		ht+='<input type="checkbox" name="general4'+custAnalysisNum+'" value="材料档次选用是否合理"/>材料档次选用是否合理';
		ht+='</td>';
		ht+='<td width="15%" align="left" >';
		ht+='<input type="checkbox" name="general4'+custAnalysisNum+'" value="工艺标准与材料档次匹配否"/>工艺标准与材料档次匹配否';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left" >';
		ht+='<input type="checkbox" name="general4'+custAnalysisNum+'" value="家具选用与装修档次匹配否"/>家具选用与装修档次匹配否';
		ht+='</td>';
		ht+='<td width="15%" align="left" >';
		ht+='<input type="checkbox" name="general4'+custAnalysisNum+'" value="适当采用特殊工艺提高档次"/>适当采用特殊工艺提高档次';
		ht+='</td>';
		ht+='<td width="15%" align="left" >';
		ht+='<input type="checkbox" name="general4'+custAnalysisNum+'" value="选择确定家用电器档次"/>选择确定家用电器档次';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left" >';
		ht+='<input type="checkbox" name="general4'+custAnalysisNum+'" value="选择确定洁具档次"/>选择确定洁具档次';
		ht+='</td>';
		ht+='<td width="15%" align="left" >';
		ht+='<input type="checkbox" name="general4'+custAnalysisNum+'" value="后期软装饰和配饰确定"/>后期软装饰和配饰确定';
		ht+='</td>';
		ht+='<td width="15%" align="left" >';
		ht+='<input type="checkbox" name="general4'+custAnalysisNum+'" value="必要时是否附效果图"/>必要时是否附效果图';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" >';
		ht+='<td width="15%" align="center" width="3%" style="border-left:1px solid  #b9d8f3;border-right:1px solid #b9d8f3;border-bottom:1px solid  #b9d8f3;">下一步措施及方案</td>';
		ht+='<td align="center" border="0" width="45%" style="border-right:1px solid  #b9d8f3;">';
		ht+='<table style="padding:5px"  border="1" cellpadding="3" cellspacing="1" width="100%" align="center" style="background-color: #b9d8f3;">';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left"  colspan="4"><label style="font-weight:bold;">设计师下一步服务措施：</label><textarea id="des_next_measure'+custAnalysisNum+'" cols="90" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left"  colspan="4"><label style="font-weight:bold;">客户经理下一步措施：</label><textarea id="cusm_next_measure'+custAnalysisNum+'" cols="90" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left"  colspan="4"><label style="font-weight:bold;">其它措施：</label><textarea id="other_measure'+custAnalysisNum+'" cols="90" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		ht+='</tr>';
		...
		ht+='<table border="1" cellpadding="3" cellspacing="1" width="100%" align="center" style="background-color: #b9d8f3;">';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="5%" align="center">序号</td>';
		ht+='<td width="30%" align="center">分析项目</td>';
		ht+='<td width="65%" align="center">分析目的</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">1</td>';
		ht+='<td align="right" style="padding-right:5px">年龄与性格:</td>';
		ht+='<td align="left"><textarea id="cy1'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">2</td>';
		ht+='<td align="right" style="padding-right:5px">职业:</td>';
		ht+='<td align="left"><textarea id="cy2'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">3</td>';
		ht+='<td align="right" style="padding-right:5px">咨询人（之间）关系:</td>';
		ht+='<td align="left"><textarea id="cy3'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">4</td>';
		ht+='<td align="right" style="padding-right:5px">现居住地址:</td>';
		ht+='<td align="left"><textarea id="cy4'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">5</td>';
		ht+='<td align="right"style="padding-right:5px">着装:</td>';
		ht+='<td align="left"><textarea id="cy5'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">6</td>';
		ht+='<td align="right" style="padding-right:5px">代步工具:</td>';
		ht+='<td align="left"><textarea id="cy6'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">7</td>';
		ht+='<td align="right" style="padding-right:5px">装修房屋的用途:</td>';
		ht+='<td align="left"><textarea id="cy7'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">8</td>';
		ht+='<td align="right" style="padding-right:5px">个性需求:</td>';
		ht+='<td align="left"><textarea id="cy8'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr><tr class="tr_data" height="35px">';
		ht+='<td align="center">9</td>';
		ht+='<td align="right" style="padding-right:5px">第几次置业:</td>';
		ht+='<td align="left"><textarea id="cy9'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">10</td>';
		ht+='<td align="right" style="padding-right:5px">第几次装修:</td>';
		ht+='<td align="left"><textarea id="cy10'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">11</td>';
		ht+='<td align="right" style="padding-right:5px">其他品牌的了解:</td>';
		ht+='<td align="left"><textarea id="cy11'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">12</td>';
		ht+='<td align="right" style="padding-right:5px">客户的期望值与体验后的落差:</td>';
		ht+='<td align="left" ><textarea id="cy12'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr><tr class="tr_data" height="35px">';
		ht+='<td align="center">13</td>';
		ht+='<td align="right" style="padding-right:5px">客户对设计师的评价:</td>';
		ht+='<td align="left"><textarea id="cy13'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr><tr class="tr_data" height="35px">';
		ht+='<td align="center">14</td>';
		ht+='<td align="right" style="padding-right:5px">决策人:</td>';
		ht+='<td align="left"><textarea id="cy14'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">15</td>';
		ht+='<td align="right" style="padding-right:5px">客户是否愿意一站式产品服务:</td>';
		ht+='<td align="left"><textarea id="cy15'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">16</td>';
		ht+='<td align="right" style="padding-right:5px">客户是否愿意选择同一品牌的产品:</td>';
		ht+='<td align="left"><textarea id="cy16'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">17</td>';
		ht+='<td align="right" style="padding-right:5px">客户愿意定制化装修还是套餐模式:</td>';
		ht+='<td align="left"><textarea id="cy17'+custAnalysisNum+'" cols="80" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='<br/>';
		ht+='<B>客户特殊需求记录表</B>';
		...
		ht+='<table border="1" cellpadding="3" cellspacing="1" width="100%" align="center" style="background-color: #b9d8f3;">';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" colspan="2">特殊要求详细内容</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" width="15%">合同方面</td>';
		ht+='<td align="left"><textarea id="special1'+custAnalysisNum+'" style="width:720" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" >设计方面</td>';
		ht+='<td align="left"><textarea id="special2'+custAnalysisNum+'" style="width:720" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" >材料方面</td>';
		ht+='<td align="left"><textarea id="special3'+custAnalysisNum+'" style="width:720" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" >工艺方面</td>';
		ht+='<td align="left"><textarea id="special4'+custAnalysisNum+'" style="width:720" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" >服务方面</td>';
		ht+='<td align="left"><textarea id="special5'+custAnalysisNum+'" style="width:720" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" >其它方面</td>';
		ht+='<td align="left"><textarea id="other'+custAnalysisNum+'" style="width:720" rows="2"></textarea></td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='<br/>';
		
		$("#custAnalysis"+custAnalysisNum).html(ht);
		//view
		ht='';
		ht+='<table border="0" cellpadding="3" cellspacing="1px" width="100%" align="center" style="background-color: #b9d8f3;border-collapse:collapse;" >';
		ht+='<tr class="tr_data" >';
		...
		ht+='<table width="100% frame=void">';
		ht+='<tr>';
		ht+='<td colspan="3" width="99%" id="view_is_consume_ability'+custAnalysisNum+'">';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>	';
		...
		ht+='<table width="100%" frame=void>';
		ht+='<tr>';
		ht+='<td width="99%" colspan="3" id="view_cust_focus'+custAnalysisNum+'">';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr>';
		ht+='<td width="99%" id="view_cust_focus'+custAnalysisNum+'">';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		...
		ht+='<table width="100%" frame=void>';
		ht+='<tr>';
		ht+='<td width="99%" colspan="3" id="view_cust_lost_reason'+custAnalysisNum+'">';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		...
		ht+='<td border="0" width="15%" align="left"  colspan="3">';
		ht+='<label style="font-weight:bold;">客户选择我们公司的可能性在：</label>';
		ht+='<label id="view_choose_reason'+custAnalysisNum+'">';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left"  colspan="3">';
		ht+='<label style="font-weight:bold;">客户对我们公司整体印象是：</label><span id="view_general1'+custAnalysisNum+'"></span>';
		ht+='</td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left"  colspan="3">';
		ht+='<label style="font-weight:bold;">客户对设计师的整体印象是：</label><span id="view_general2'+custAnalysisNum+'"></span>';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		...
		ht+='<tr class="tr_data" >';
		ht+='<td width="15%" align="center" width="3%" style="border-left:1px solid  #b9d8f3;border-bottom:1px solid  #b9d8f3;">设计思路和平面布置</td>';
		ht+='<td align="left" width="45%" style="padding:0px;border-collapse:collapse;">';
		ht+='<table border="0"  width="100%" align="center" style="background-color: #b9d8f3;border-collapse:collapse;">';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" colspan="3" id="view_general3'+custAnalysisNum+'" align="left"  style="border-left:1px solid #b9d8f3;border-right:1px solid #b9d8f3;">';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		...
		ht+='<table border="1" cellpadding="3" cellspacing="1" width="100%" align="center" style="background-color: #b9d8f3;">';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" colspan="3" id="view_general4'+custAnalysisNum+'" align="left" >';
		ht+='</td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		...
		ht+='<table style="padding:5px"  border="1" cellpadding="3" cellspacing="1" width="100%" align="center" style="background-color: #b9d8f3;">';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left"  colspan="4"><label style="font-weight:bold;">设计师下一步服务措施：</label><label id="view_des_next_measure'+custAnalysisNum+'"></label></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left"  colspan="4"><label style="font-weight:bold;">客户经理下一步措施：</label><label id="view_cusm_next_measure'+custAnalysisNum+'"></label></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td width="15%" align="left"  colspan="4"><label style="font-weight:bold;">其它措施：</label><label id="view_other_measure'+custAnalysisNum+'"></label></td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='</td>';
		ht+='</tr>';
		...
		ht+='<td align="center">1</td>';
		ht+='<td align="right" style="padding-right:5px">年龄与性格:</td>';
		ht+='<td align="left" id="view_cy1'+custAnalysisNum+'" ></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">2</td>';
		ht+='<td align="right" style="padding-right:5px">职业:</td>';
		ht+='<td align="left" id="view_cy2'+custAnalysisNum+'" ></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">3</td>';
		ht+='<td align="right" style="padding-right:5px">咨询人（之间）关系:</td>';
		ht+='<td align="left" id="view_cy3'+custAnalysisNum+'" ></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">4</td>';
		ht+='<td align="right" style="padding-right:5px">现居住地址:</td>';
		ht+='<td align="left" id="view_cy4'+custAnalysisNum+'"></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">5</td>';
		ht+='<td align="right"style="padding-right:5px">着装:</td>';
		ht+='<td align="left" id="view_cy5'+custAnalysisNum+'"></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">6</td>';
		ht+='<td align="right" style="padding-right:5px">代步工具:</td>';
		ht+='<td align="left" id="view_cy6'+custAnalysisNum+'"></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">7</td>';
		ht+='<td align="right" style="padding-right:5px">装修房屋的用途:</td>';
		ht+='<td align="left" id="view_cy7'+custAnalysisNum+'" ></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">8</td>';
		ht+='<td align="right" style="padding-right:5px">个性需求:</td>';
		ht+='<td align="left" id="view_cy8'+custAnalysisNum+'"></td>';
		ht+='</tr><tr class="tr_data" height="35px">';
		ht+='<td align="center">9</td>';
		ht+='<td align="right" style="padding-right:5px">第几次置业:</td>';
		ht+='<td align="left" id="view_cy9'+custAnalysisNum+'" ></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">10</td>';
		ht+='<td align="right" style="padding-right:5px">第几次装修:</td>';
		ht+='<td align="left" id="view_cy10'+custAnalysisNum+'" ></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">11</td>';
		ht+='<td align="right" style="padding-right:5px">其他品牌的了解:</td>';
		ht+='<td align="left" id="view_cy11'+custAnalysisNum+'"></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">12</td>';
		ht+='<td align="right" style="padding-right:5px">客户的期望值与体验后的落差:</td>';
		ht+='<td align="left" id="view_cy12'+custAnalysisNum+'" ></td>';
		ht+='</tr><tr class="tr_data" height="35px">';
		ht+='<td align="center">13</td>';
		ht+='<td align="right" style="padding-right:5px">客户对设计师的评价:</td>';
		ht+='<td align="left" id="view_cy13'+custAnalysisNum+'"></td>';
		ht+='</tr><tr class="tr_data" height="35px">';
		ht+='<td align="center">14</td>';
		ht+='<td align="right" style="padding-right:5px">决策人:</td>';
		ht+='<td align="left" id="view_cy14'+custAnalysisNum+'" ></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">15</td>';
		ht+='<td align="right" style="padding-right:5px">客户是否愿意一站式产品服务:</td>';
		ht+='<td align="left" id="view_cy15'+custAnalysisNum+'" ></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">16</td>';
		ht+='<td align="right" style="padding-right:5px">客户是否愿意选择同一品牌的产品:</td>';
		ht+='<td align="left" id="view_cy16'+custAnalysisNum+'" ></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center">17</td>';
		ht+='<td align="right" style="padding-right:5px">客户愿意定制化装修还是套餐模式:</td>';
		ht+='<td align="left" id="view_cy17'+custAnalysisNum+'" ></td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='<br/>';
		ht+='<B>客户特殊需求记录表</B>';
		...
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" width="15%">合同方面</td>';
		ht+='<td align="left" id="view_special1'+custAnalysisNum+'"></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" >设计方面</td>';
		ht+='<td align="left" id="view_special2'+custAnalysisNum+'"></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" >材料方面</td>';
		ht+='<td align="left" id="view_special3'+custAnalysisNum+'"></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" >工艺方面</td>';
		ht+='<td align="left" id="view_special4'+custAnalysisNum+'"></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" >服务方面</td>';
		ht+='<td align="left" id="view_special5'+custAnalysisNum+'"></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="center" >其它方面</td>';
		ht+='<td align="left" id="view_other'+custAnalysisNum+'"></td>';
		ht+='</tr>';
		ht+='</table>';
		ht+='<br/>';
	
		$("#view_custAnalysis"+custAnalysisNum).html(ht);
		
		custAnalysisNum++;
	}
	function showCustAnalyze(){
		var num=cur_tab_id;
		$("#custAnalysis"+num).show();
		$("#view_custAnalysis"+num).hide();
		saveCBModelBt();
	 }
...
//---------------------------客户回访-------------------------
	...
	function AddMoreCallBack(){
		...
		ht+='<tr class="tr_data" height="35px">';
			...
			ht+='<td width="25%" align="center" width="100px">本次来店时间';
			ht+='<input type="hidden" id="next_id'+callbackNum+'"></td>';
			ht+='<td width="25%" align="left"><input type="text" id="come_time'+callbackNum+'" onclick="new Calendar().show(this);" /></td>';
			ht+='<td width="25%" align="center" >预约下次来店时间</td>';
			ht+='<td width="25%" align="left" ><input type="text" id="next_time'+callbackNum+'" onclick="new Calendar().show(this);" /></td>';
			ht+='</tr>';
					ht+='<tr class="tr_data" height="35px">';
					ht+='<td align="center">客户满意度<span style="color:red;">(*)</span></td>';
					ht+='<td align="left" colspan="3"><span id="tdcust_sati'+callbackNum+'" style="padding:0px 15px 0px 0px" ></span><span style="display:none" id="tdunsati_reason'+callbackNum+'"></span></td>';
		...
		ht+='<tr class="tr_data" height="35px">';
			ht+='<td align="center" >电话回访时间</td>';
			ht+='<td align="left"><input type="text" id="callback_time'+callbackNum+'" onclick="new Calendar().show(this);" /></td>';
			ht+='<td align="center">结果达成</td>';
			ht+='<td align="left" id="tdresult_status'+callbackNum+'"></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
			ht+='<td align="center">预约量房时间</td>';
			ht+='<td align="left"><input type="text" id="measure_date'+callbackNum+'" onclick="new Calendar().show(this);" value="'+top.getTimeStr(measureHouseDate,true)+'"/></td>';
			ht+='<td align="center">客户意向</td>';
			ht+='<td align="left" id="tdcust_intention'+callbackNum+'"></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
			ht+='<td align="center" >客户意见或建议</td>';
			ht+='<td align="left" colspan="3"><textarea style="width:680px" id="cust_suggest'+callbackNum+'" rows="3" cols="70"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
			ht+='<td align="center">销售策略</td>';
			ht+='<td align="left" colspan="3"><textarea style="width:680px" id="sales_measure'+callbackNum+'" rows="3" cols="70"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
			ht+='<td align="center"><a href="javascript:uploadSalesMeasureFile(\''+callbackNum+'\')">其他销售策略上传附件</a></td>';
			ht+='<td align="left" colspan="3"><div id="lab_file_path'+callbackNum+'"></div><input type="hidden" id="file_path'+callbackNum+'" size="20"/></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data" height="35px">';
			ht+='<td align="center">短信内容</td>';
		...
		ht+='<tr class="tr_data">';
		ht+='<td class="table_property_title" style="width:110px;">客户经理</td>';
		ht+='<td align="left"><textarea id="cusm_callback_info'+callbackNum+'" rows="2" style="width:100%"></textarea></td>';
		ht+='</tr>';
		ht+='<tr class="tr_data">';
		ht+='<td class="table_property_title" style="width:110px;">设计师</td>';
		...
		ht+='<table border="1" cellpadding="3" cellspacing="1" width="100%" align="center" style="background-color: #b9d8f3;">';
		ht+='<tr class="tr_data" height="35px">';
		ht+='<td align="left"><textarea id="custom_tactics'+callbackNum+'" rows="5" style="width:800px"></textarea></td>';
		ht+='</tr>';
		ht+='</table>';
		...
		ht+='<tr class="tr_data" height="35px">';
			...
			ht+='<td width="25%" align="center">本次来店时间</td>';
	 ...
	 try{
        if ( obj.status == true ){
			...
			if(cur_tab_id==callbackNum-1){
				...
				$("#view_file_path"+cur_tab_id).html( $("#file_path"+cur_tab_id).val());
				//$("#addTrBtn").show();
			}
        }else{
            isSaveOK = false;
	...
	function showCallBackInfo(showDivName){
		callBackOrAnalysis=showDivName;
		callIndex=cur_tab_id;
		if(showDivName=='callInfo'){
		//判断打开的便签是否为最新一次，最新一次显示编辑状态
			/*if(callIndex!=(callbackNum-1)){
				$("#custAnalysis"+callIndex).hide();
				$("#callBackInfo"+callIndex).hide();
				$("#view_custAnalysis"+callIndex).hide();
				$("#view_callBackInfo"+callIndex).show();
			}else{*/
				$("#custAnalysis"+callIndex).hide();
				$("#callBackInfo"+callIndex).hide();
				$("#view_custAnalysis"+callIndex).hide();
				$("#view_callBackInfo"+callIndex).show();	
			//}
			$("#left_menu1_"+callIndex).removeClass("versionStyle2");
			$("#left_menu2_"+callIndex).removeClass("versionStyle");
			$("#left_menu2_"+callIndex).addClass("versionStyle2");
			$("#left_menu1_"+callIndex).addClass("versionStyle");
		}else if(showDivName=='analysisInfo'){
			if(callIndex!=(callbackNum-1)){
				$("#custAnalysis"+callIndex).hide();
				$("#callBackInfo"+callIndex).hide();
				$("#view_custAnalysis"+callIndex).show();
				$("#view_callBackInfo"+callIndex).hide();
			}else{
				$("#custAnalysis"+callIndex).show();
				$("#callBackInfo"+callIndex).hide();
				$("#view_custAnalysis"+callIndex).hide();
				$("#view_callBackInfo"+callIndex).hide();
			}
			$("#left_menu1_"+callIndex).removeClass("versionStyle");
			$("#left_menu2_"+callIndex).removeClass("versionStyle2");
			$("#left_menu2_"+callIndex).addClass("versionStyle");
			$("#left_menu1_"+callIndex).addClass("versionStyle2");
		}
		changBaseBtclick(cur_tab_id,callBackOrAnalysis);
	}
   ...
   //点击不同标签页，按钮绑定不同触发事件
    function changBtBind(tabTitle){
		if(tabTitle=="custBase"){//客户基本信息
		...
		}else if(tabTitle=='contract'){//工程合同
		    baseDoCancel();
			callbackPlanDoCancel();
			//合同审核
		    ...
		    $("#baseEditBt").attr("onclick","editContractInfo();");
			showProjectContractInfo();
		}else if(tabTitle=='callbackPlan'){//回访计划
			 cancleContract();
			 baseDoCancel(); 
			 if(top.g_isPermit("09_cust_callback_opt")){
			     $("#callback_plan").show();
				 //$("#baseSaveBt").attr("onclick","javascript:callbackPlanDoSubmit();");
				 //$("#baseReturnBt").attr("onclick","javascript:callbackPlanDoCancel();");
				 //$("#baseEditBt").attr("onclick","callbackPlanDoEdit();");
				 //callbackPlanDoCancel();
			 }else{
			     $("#callback_plan").hide();
				 //$("#baseEditBt").hide();
	...
	function editContractInfo(){
	  if(top.g_isPermit('project_contract_opr')){
		...
		//$("#baseSaveBt").show();
		//$("#baseReturnBt").show();
		//$("#baseSaveBt").attr("onclick","javascript:saveContractInfo();");
		//$("#baseReturnBt").attr("onclick","javascript:cancleContract();");
		$("#contractEditBt").hide();
		$("#contractSaveBt").show();
		...
		$("#project_contract_info input").show();
		$("#project_process_list input").hide();
		$("#project_forfeit_List input").hide();
		$("#project_contract_info span").hide();
		...
		$("#tdcontract_check").html(top.getCodeSelectHtml("EXAMINE_STATUS","contract_check_status",true,$("#check_status").val()));
	...
	function cancleContract(){
		...
		$("#spancontract_check_memo").html($("#contract_check_memo").val());
		$("#spancontract_check_memo").show();
		$("#check_button").hide();
		//当审核信息为空隐藏审核表 当违款记录为空 隐藏违款记录表
		if($("#project_forfeit_List tr").length>0){
			$("#project_forfeit_List").show();
			 $("#weiyueTitle").show();
		}else{
	...
	function examineContract(){
	   ...
	   aoData.push({ "name":"contract.check_status","value":$("#contract_check_status").val()});
	...
	function examineContractCallback(obj){
	   if(obj.status==true){
	      top.showInfoWinOK("操作成功");
		  $("#check_status").val($("#contract_check_status").val());
		  cancleContract();
	   }else{
	      top.showInfoWinError("操作失败-->"+obj.msg);
	   }
	...
	function showProjectContractInfo(){
		if(!isProjectContractShowed){
			...
			$("#custManageWorkTab li:eq(0) a").trigger("click");
		}
	}	
	//若有工程合同信息则获取工程合同信息
	...
	function getProjectContractInfoCallBack(obj){
	    ...
	    if(obj.status==true){
			...
			$("#project_forfeit_List input").hide();
			var result=(o.first_is_payed==1||o.first_is_payed==2);
			if(result){
			  $("#first_pay_img").addClass("doApply");
			}
			
			result=(o.second_is_payed==1||o.second_is_payed==2);
			if(result){
			  $("#second_pay_img").addClass("doApply");
			}

			result=(o.third_is_payed==1||o.third_is_payed==2);
			if(result){
...
//doQuery---上传文件
	...
	function saveMoneyRecharge(){
		var msg='';
			if ( $("#money_value").val() != '' && !IsFloat( $("#money_value").val() )) {
				msg += "[充值金额]应为数字类型<br>";
				$("#money_value").val('0');
			}
			if ( msg != '' ) {
